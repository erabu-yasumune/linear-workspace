...
export const bulkIssueRowSchema = z.object({
  ...
  estimate: z
    .number()
    .refine(
      (val) => ESTIMATE_OPTIONS.includes(val as (typeof ESTIMATE_OPTIONS)[number]),
      {
  ...
  assigneeId: z.string().uuid("無効なAssignee IDです").optional().or(z.literal("")),
  parentId: z.string().uuid("無効なParent IDです").optional().or(z.literal("")),
});

export const bulkIssueInputSchema = z.array(bulkIssueRowSchema).min(1, "少なくとも1つのIssueを入力してください");
